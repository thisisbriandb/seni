  ...
  return (
    <MotionWrapper>
      <main>
        ...
        <motion.div 
          ref={heroRef}
          className="relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary/10 to-accent/10"
          style={{ opacity }}
        >
          ...
          >
            <FloatingElement>
              <motion.h1 
                className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold mb-4 sm:mb-6 bg-clip-text text-transparent bg-gradient-to-r from-primary to-accent"
            ...
            <FloatingElement delay={0.2}>
              <motion.p 
                className="text-base sm:text-lg md:text-xl lg:text-2xl mb-6 sm:mb-8 text-text-secondary"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            <FloatingElement delay={0.4}>
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="px-5 sm:px-6 md:px-8 py-3 sm:py-4 bg-gradient-to-r from-primary to-accent text-white rounded-full text-sm sm:text-base font-semibold shadow-lg hover:shadow-xl inline-flex items-center gap-1 sm:gap-2 group"
              >
        ...
        {/* Statistics Section */}
        <ParallaxSection className="py-10 sm:py-12 md:py-16 bg-gradient-to-br from-primary/5 to-accent/5" decorative>
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
              {statistics.map((stat, index) => (
                <FloatingElement key={index} delay={index * 0.1}>
                  <motion.div
                    whileHover={{ y: -5 }}
                    className="bg-white/80 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 shadow-md sm:shadow-lg text-center"
                  >
                    <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-2 sm:mb-4">
                      <Icon icon={stat.icon} className="text-lg sm:text-xl md:text-2xl text-primary" />
                    </div>
                    <motion.div
                      initial={{ scale: 0 }}
                      whileInView={{ scale: 1 }}
                      transition={{ type: "spring", duration: 0.8, delay: index * 0.1 }}
                      className="text-xl sm:text-2xl md:text-3xl font-bold text-primary-dark mb-1 sm:mb-2"
                    >
                      {stat.value}
                    </motion.div>
                    <p className="text-xs sm:text-sm md:text-base text-text-secondary">{stat.label}</p>
                  </motion.div>
                </FloatingElement>
              ))}
            </div>
        ...
        {/* FAQ Section */}
        <ParallaxSection className="py-12 sm:py-16 md:py-20" decorative>
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              <FloatingElement>
                <div className="text-center mb-8 md:mb-12">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-primary-dark mb-2 sm:mb-4">
                ...
                {faqs.map((faq, index) => (
                  <FloatingElement key={index} delay={index * 0.1}>
                    ...
                    >
                      ...
                      >
                        <span className="font-medium text-sm sm:text-base text-primary-dark pr-4">{faq.question}</span>
                        <Icon
                          icon="ri:arrow-down-s-line"
                          className={`text-lg sm:text-xl text-primary transition-transform ${
                      ...
                      >
                        <div className="px-4 sm:px-6 pb-3 sm:pb-4 text-xs sm:text-sm text-text-secondary">
                          {faq.answer}
                        </div>
                      </motion.div>
                    </motion.div>
        ...
        <ParallaxSection className="py-10 sm:py-12 md:py-16 bg-gradient-to-br from-primary/5 to-accent/5" decorative>
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              <FloatingElement>
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-primary-dark mb-3 sm:mb-4 md:mb-6">
                  Besoin d'informations supplémentaires ?
                </h2>
                <p className="text-sm sm:text-base text-text-secondary mb-5 sm:mb-6 md:mb-8 px-2 sm:px-0">
                  Notre équipe est disponible 24/7 pour répondre à toutes vos questions et vous accompagner dans votre démarche.
                </p>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 md:py-3 bg-accent text-white rounded-full text-xs sm:text-sm md:text-base font-medium shadow-md hover:shadow-lg inline-flex items-center gap-1 sm:gap-2"
                >

...
export default function InteractiveGlobe() {
  ...
  const isMounted = useRef(false); // Réf pour suivre l'état du montage
  ...
  useEffect(() => {
    isMounted.current = true; // Composant monté
    updateDimensions();
    window.addEventListener("resize", updateDimensions);
    return () => {
      isMounted.current = false; // Composant démonté
      window.removeEventListener("resize", updateDimensions);
    };
  }, [updateDimensions]);

  useEffect(() => {
    if (globeIsReady && isMounted.current) {
  ...
  useEffect(() => {
    if (globeRef.current && globeIsReady && isMounted.current) {
  ...
  const handleLocationClick = (
    ...
    if (globeRef.current) {
      ...
      if (location.name === "Bangui")
        zoomAltitude = isMobile
          ? orientation === "landscape"
            ? 1.8
            : 2.0
          : 1.8;
      ...
      globeRef.current.pointOfView(
        { lat: location.lat, lng: location.lng, altitude: zoomAltitude },
        1000
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      <DynamicGlobe
        ...
        pointAltitude={0.05}
        pointRadius={(d) =>
          isMobile ? (d as Location).size * 0.8 : (d as Location).size
        }
        pointLabel={(d) => `
          <div class="bg-white/90 backdrop-blur-md p-2 rounded-lg shadow-lg ${
            ...
            <div class="flex items-center gap-2">
              ...
              <div>
                ...
                } font-bold text-primary">${(d as Location).name}</div>
                <div class="${
                  isMobile ? "text-xs" : "text-sm"
                } text-gray-600">${(d as Location).country}</div>
              </div>
            </div>
          </div>
        `}
      ...
      {activeLocation && (
        ...
        >
          <h3 className="text-sm sm:text-lg font-bold text-primary">
            {activeLocation.name}
          </h3>
